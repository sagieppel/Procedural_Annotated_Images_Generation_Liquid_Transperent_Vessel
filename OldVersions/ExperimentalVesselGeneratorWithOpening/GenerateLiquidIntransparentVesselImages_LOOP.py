...
def CreateSlope(slope0):
    ...
    if pr<0.25:
        ...
        Drad=np.random.rand()*np.random.rand()*np.random.rand()*1#0.3
        slope=0
        b=0
    y=0
...
# CreateRadius Array for vessel profile
...
#                     continue
        ...
        return rl, MaterialTopHeight, MaterialInitHeight   
##################################################################################################################333

#                          Create vessel instance

####################################################################################################################
def AddVessel(VesselName="Vessel",ContentName="MaterialContent",MinH=5,MaxH=80,MinR=15,MaxR=60):   
    print("=================Create Vessel Mesh object and add to scene==================================")     
    #--------------------Create random shape vessel assign parameters----------------------------------------- 
    
    MatX_RadRatio=MatY_RadRatio=np.random.rand()*0.9+0.05 # Ratio of the material radius compare to 
    ...
    # Generate
    rl, MaterialTopHeight, MaterialInitHeight=CreateRadiusArray(MinH,MaxH,MinR,MaxR)      
    ...
    for fz in range(len(rl)):
        for j in range(0,Vnum ):
         ...
         #   print(x)
            ...
            if fz<=MaterialTopHeight and fz>=MaterialInitHeight: # Material inside vessel
    ...
    #********************Vessel floor second option************************
        for k in range(0,Vnum): 
                face = (k,(k+1)%Vnum,(k+int(Vnum/2))%Vnum)#
                faces.append(face) 
                Matfaces.append(face)
                Openfaces.append(face)
    
    #------------material top as as a sing single face-------------------------------------------        
...
OutFolder=r"C:\Users\Sagi\Desktop\Procedural_Annotated_Images_Generation_Liquid_Transperent_Vessel/Out/"# Where output images will be saved
HDRI_BackGroundFolder=r"C:\Users\Sagi\Desktop\Procedural_Annotated_Images_Generation_Liquid_Transperent_Vessel/HDRI_BackGround//" # Background dri folder
PBRMaterialsFolder=r"C:\Users\Sagi\Desktop\Procedural_Annotated_Images_Generation_Liquid_Transperent_Vessel/PBRMaterials/" #folder of pbr materials
